...
/**
 * 数据库监控自动装配（SB3）：注册 SQL 统计注册表、数据源监控登记表、
 * 数据源包装 BeanPostProcessor、Hikari 池指标贡献者（按 classpath 条件）与 DB 端点控制器。
 * <p>装配条件：{@code @ConditionalOnWebApplication} 不限类型 +
 * {@code archimedes.api.enabled} 与 {@code archimedes.sql.enabled} 双开关（均默认开启）。
 * 监控范围为 JDBC {@code DataSource}（R2DBC 不在本期范围）。
 *
 * @author nianliu-jj
 * @since 2026-07-08
 */
