# spring-boot-3-support Specification

## Purpose

定义 `archimedes-spring-boot-3-starter` 在 Spring Boot 3.x（jakarta 环境）下的引入即用能力：零配置自动装配（与拆分前单模块行为等价）、AutoConfiguration.imports 注册方式与属性开关。
